...
class paging
{
	...
	function assign ( $url , $total_records , $records_per_page = false, $page_no=0  )
	{
		...
		$this->current_page = ( $page_no ) ? $page_no : '1';
	...
	function generate_pages ()
	{
		...
		while ( $i <= $end_page )
		{
			if ( $i != $this->current_page )
			{
				...
				$pagearray[] = '<li '.$this->active_page_class.'><a>'.$i.'</a></li>';
	...
	function generate_link ( $inner, $page_number )
	{
		...
		$link = '<li><a href="'.$url.'"'.$onclick.'>'.$inner.'</a></li>';
