		...
		jQuery(document).ready(function(){
			...
			jQuery("#mapnew").css("height",(map_height-20));
			 ...
			 jQuery('#add').click(function() { 
				 navigator.geolocation.getCurrentPosition(draw_cust_marker);
				 
			}); 
			
			  
			  function draw_cust_marker(position){
				  jQuery.goMap.removeOverlay('circle', 'riga');
					...
					jQuery.goMap.removeMarker('cust_marker');
				...
				jQuery.goMap.createListener({type:'marker', marker:'cust_marker'}, 'dragend', function() { 
        ...
        foreach ($fivesdrafts as $author)
        {
            ...
            if (trim($author_info->perma_lat) != '' && trim($author_info->perma_long) != '')
            {
                ...
                $map_arr[] = "{  
                        ...
                        id: 'map_" . $i . "', 
                        icon: '".$icon."',
						html: { content: '<div class=\"row\" ><div class=\"col-xs-12 text-center\"><img src=\"".get_noone_meta($author_info->ID,'gomap_marker_html')."\" ></div><div class=\"col-xs-12 text-center distance_text\">".round($author->distance,2)." Km from you</div><div class=\"col-xs-12 text-center\"><a href=\"javascript:void(0);\" onClick=\"javascript:info_show(\'".$author_info->first_name." ".$author_info->last_name."\',".$author_info->ID.",450,800);\" class=\"btn btn-primary btn-sm\">VIEW MORE</a></div></div>'}
                        }";
            } 
        } 
...
if ($fivesdrafts)
    ...
    <div class="container">
        ...
        <input name="add" value="Add marker" id="add" type="button">
        </div>
        <ul class="ul_user" > 
 <?php       $i = 0;
        foreach ($fivesdrafts as $author)
