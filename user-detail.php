		  ...
		  jQuery.ajax({
			...
			success: function(data, textStatus, XMLHttpRequest){
			  ...
			  if(data=='0'){
				...
				jQuery('#mail_response').html('<div class="mail_response"><?php echo get_user_meta($aID, 'first_name', true).' '.get_user_meta($aID, 'last_name', true);?> will contact you asap.</div>');
				jQuery("#send_message_btn").val('Send');
				jQuery("#send_message_btn").removeAttr("disabled");
				info_remove();
				setTimeout(function(){ 
					jQuery('#mail_response').slideUp("slow").html("");						
				}, 3000);
